...
var N3 = {
     ...
     **/
    States: {
        AUTHENTICATION: 1, TRANSACTION: 2, UPDATE: 3
    },
    ...
    capabilities: {
        // AUTHENTICATION
        1: ["UIDL", "USER", "RESP-CODES", "AUTH-RESP-CODE"], // TRANSACTION
        2: ["UIDL", "EXPIRE NEVER", "LOGIN-DELAY 0", "IMPLEMENTATION N3 node.js POP3 server"], // UPDATE
        3: []
    },
    ...
    startServer: function (port, server_name, auth, MsgStore, callback) {
        ...
        net.createServer(this.createInstance.bind(this, server_name, auth, MsgStore)).listen(port, function (err) {
            if (err) {
...
N3.POP3Server = function (socket, server_name, auth, MsgStore) {
    ...
    this.capabilities = {
        1: Object.create(N3.capabilities[1]), 2: Object.create(N3.capabilities[2]), 3: Object.create(N3.capabilities[3])
    }
...
N3.POP3Server.prototype.updateTimeout = function () {
    ...
    this.timer = setTimeout((function () {
        ...
        if (this.sate == N3.States.TRANSACTION) this.state = N3.States.UPDATE;
        //console.log("Connection closed for client inactivity\n\n");
        if (this.user && N3.connected_users[this.user.trim().toLowerCase()]) delete N3.connected_users[this.user.trim().toLowerCase()];
        this.socket.end();
        this.destroy();
...
N3.POP3Server.prototype.afterLogin = function () {
    ...
    if (typeof this.MsgStore != "function") return false;
...
N3.POP3Server.prototype.onCommand = function (request) {
    var cmd = request.match(/^[A-Za-z]+/), params = cmd && request.substr(cmd[0].length + 1);
    ...
    if (!cmd) return this.response("-ERR");
...
N3.POP3Server.prototype.cmdCAPA = function (params) {
    ...
    if (N3.authMethods) {
        ...
        for (var i in N3.authMethods) {
            if (N3.authMethods.hasOwnProperty(i)) methods.push(i);
        }
        if (methods.length && this.state == N3.States.AUTHENTICATION) this.response("SASL " + methods.join(" "));
    }
    this.response(".");
}
...
N3.POP3Server.prototype.cmdAUTH = function (auth) {
    ...
    if (!auth) return this.response("-ERR Invalid authentication method");

    var parts = auth.split(" "), method = parts.shift().toUpperCase().trim(), params = parts.join(" "), response;
...
N3.POP3Server.prototype.cmdAUTHCheck = function (user, passFn) {
    ...
    if (typeof this.authCallback == "function") {
        if (typeof passFn == "function") return !!this.authCallback(user, passFn); else if (typeof passFn == "string" || typeof passFn == "number") return !!this.authCallback(user, function (pass) {
            return pass == passFn
        }); else return false;
    }
    return true;
...
N3.POP3Server.prototype.cmdAPOP = function (params) {
    ...
    params = params.split(" ");
    var user = params[0] && params[0].trim(), hash = params[1] && params[1].trim().toLowerCase(),
        salt = "<" + this.UID + "@" + this.server_name + ">", response;
    ...
    } else return this.response(response || "-ERR [SYS] Error with initializing");
}
...
N3.POP3Server.prototype.cmdUSER = function (username) {
    ...
    if (!this.user) return this.response("-ERR User not set, try: USER <username>");
...
N3.POP3Server.prototype.cmdLIST = function (msg) {
    ...
    this.messages.list(msg, (function (err, list) {
        ...
        if (!list) return this.response("-ERR Invalid message ID");
